...
const runSaga = async (saga, args, mocks = [], customEffects) => {
    ...
    while(1) {
        ...
        switch(step.value && step.value.type ? step.value.type : "void") {
            case "call":
                ...
                break;
            case "fork":
                forks[forkCounter] = runSaga(step.value.fn, step.value.args, mocks);
...
module.exports = {call, fork, join, runSaga, rootSaga};