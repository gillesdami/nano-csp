...
const mocks = [{
    matcher: (step) => {
        console.log("a", step.value && step.value.type === 'put' && step.value.action === 'INCREMENT');
        console.log(step);
        return step.value && step.value.type === 'put' && step.value.action === 'INCREMENT'
    },
    value: {done: false, value: put('DECREMENT')}
}, {
    matcher: (step) => {
